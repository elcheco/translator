...
Assert::same('4 ložnice', $t->translate($message, 4));
...
Assert::same('21 спальня', $t->translate($message, 21));
Assert::same('22 спальни', $t->translate($message, 22));
Assert::same('100 спален', $t->translate($message, 100));
Assert::same('101 спальня', $t->translate($message, 101));
...
// dutch
$t->setLocale('nl_NL');
Assert::same('1 slaapkamer', $t->translate($message, 1));
Assert::same('2 slaapkamers', $t->translate($message, 2));
Assert::same('5 slaapkamers', $t->translate($message, 5));
Assert::same('100 slaapkamers', $t->translate($message, 100));

// spanish
$t->setLocale('es_ES');
Assert::same('1 dormitorio', $t->translate($message, 1));
Assert::same('2 dormitorios', $t->translate($message, 2));
Assert::same('5 dormitorios', $t->translate($message, 5));
Assert::same('100 dormitorios', $t->translate($message, 100));

// swedish
$t->setLocale('sv_SE');
Assert::same('1 sovrum', $t->translate($message, 1));
Assert::same('2 sovrum', $t->translate($message, 2));
Assert::same('5 sovrum', $t->translate($message, 5));
Assert::same('100 sovrum', $t->translate($message, 100));

// portuguese
$t->setLocale('pt_PT');
Assert::same('1 quarto', $t->translate($message, 1));
Assert::same('2 quartos', $t->translate($message, 2));
Assert::same('5 quartos', $t->translate($message, 5));
Assert::same('100 quartos', $t->translate($message, 100));

// danish
$t->setLocale('da_DK');
Assert::same('1 soveværelse', $t->translate($message, 1));
Assert::same('2 soveværelser', $t->translate($message, 2));
Assert::same('5 soveværelser', $t->translate($message, 5));
Assert::same('100 soveværelser', $t->translate($message, 100));

// norwegian
$t->setLocale('no_NO');
Assert::same('1 soverom', $t->translate($message, 1));
Assert::same('2 soverom', $t->translate($message, 2));
Assert::same('5 soverom', $t->translate($message, 5));
Assert::same('100 soverom', $t->translate($message, 100));

// hungarian
$t->setLocale('hu_HU');
Assert::same('1 hálószoba', $t->translate($message, 1));
Assert::same('2 hálószoba', $t->translate($message, 2));
Assert::same('5 hálószoba', $t->translate($message, 5));
Assert::same('100 hálószoba', $t->translate($message, 100));
